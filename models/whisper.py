...
:mod:`triton_kernels.whisper` (layernorm, row_softmax, decode_attn,
...
from triton_kernels.whisper import (
    HAVE_TRITON_KERNELS,
    batched_decode_attn,
    decode_attn,
...
class WhisperEngine:
    ...
    Accepts the ``configs/whisper.yaml`` keys directly (``model``,
    ``language``, ``max_new_tokens``, ...) so :func:`engine.model.load_leg`
