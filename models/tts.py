...
:mod:`triton_kernels.tts` accelerate the audio post-processing around the
...
from triton_kernels.tts import (
    HAVE_TRITON_KERNELS,
    conv1d_silu_fwd,
    in1d_silu_fwd,
    postprocess,
...
class KokoroEngine:
    ...
    Accepts the ``configs/tts.yaml`` keys directly (``model``, ``lang``,
    ``voice``, ``sample_rate``, ``torch_compile``, ``speed``, ``enhance``,
    ...) so :func:`engine.model.load_leg` honours the YAML.
    """
